...
function getSearchData() {
    if (typeof searchData == 'undefined') {
        return new Promise((resolve, reject) => {
            // fetch('/assets/data/search.json', {}).then(response => response.json()).then(data => {
            fetch('/assets/data/search.json', {})
                .then((response) => response.json())
                .then((data) => {
                    if (typeof articlesModel == 'undefined') {
                        searchData = data;
                        resolve(data);
                        return;
                    }
...
function updateMenu() {
    ...
    titleSet.forEach((element) => {
        switch (element.outerHTML.substring(0, 3)) {
            case '<h2':
                menuStructure += `<br><span class='t2 center'>${element.innerHTML}</span><hr>`;
                break;
            case '<h3':
